...
import city.qr.xsd.QrCodeIdParseRequest;
...
import org.apache.ws.axis2.GetQrDataById;
import org.apache.ws.axis2.GetQrDataByIdResponse;
...
@Component
public class GetQRDataById {

    static Logger logger=LoggerFactory.getLogger(GetQRDataById.class);
    @Autowired
    private SOAPConnector soapConnector;

    @Autowired
    PropConfiguration propConfig;
    public String getQRDataById(String reqData) {
        ...
        try {
            ...
            loggerImpl.info(logger,"Preparing requset data and calling API.","IN:getQRDataById");
            loggerImpl.generateProfiler("getQRDataById");
            loggerImpl.startProfiler("Preparing request data");
            
            
            JSONObject reqJsonObj = new JSONObject(reqData);
            ls_qrCodeId = reqJsonObj.getString("QRCODEID");

            if ((ls_qrCodeId == null || "".equals(ls_qrCodeId))) {
            ...
            //created request object of QrCodeIdParseRequest
            QrCodeIdParseRequest qrCodeIdParseRequest = xsdObjectFactory.createQrCodeIdParseRequest();
            
            loggerImpl.debug(logger,"Json to xml conversion done.", "IN:getQRDataById");
            
            //setting all the values in the request.
            qrCodeIdParseRequest.setPassword(xsdObjectFactory.createQrCodeIdParseRequestPassword(ls_password));
            qrCodeIdParseRequest.setQrCodeId(xsdObjectFactory.createQrCodeIdParseRequestQrCodeId(ls_qrCodeId));
            qrCodeIdParseRequest.setUsername(xsdObjectFactory.createQrCodeIdParseRequestUsername(ls_userName));

            //created object factory object.
            org.apache.ws.axis2.ObjectFactory axis2ObjectFactory = new org.apache.ws.axis2.ObjectFactory();
            //wrapping the request in GetQrdataById object.
            GetQrDataById getqrdataById = axis2ObjectFactory.createGetQrDataById();
            getqrdataById.setRequest(axis2ObjectFactory.createGetQrDataByIdRequest(qrCodeIdParseRequest));
            
            loggerImpl.debug(logger,"getQRDataById API calling", "IN:getQRDataById");
            loggerImpl.startProfiler("getQRDataById API calling.");

            //created GetQrDataByIdResponse object.
            GetQrDataByIdResponse getQrDataByIdResponse = null;
            try {
                getQrDataByIdResponse = (GetQrDataByIdResponse) soapConnector.callWebService(getqrdataById);
				...
				ls_responseData = common.ofGetErrDataJsonArray("999", 
						...
						propConfig.getMessageOfResCode("commen.exception.999","","(ENP215)"),
						ls_actualErrMsg, "Currently Service under maintenance so please try later (ENP215)", "0", "R");

				ls_actualErrMsg = common.ofGetTotalErrString(soapException, "");
				loggerImpl.error(logger,"SoapFaultClientException : " + ls_actualErrMsg,"EX:getQRDataById");
				return ls_responseData;
			}
            
            loggerImpl.debug(logger,"getQRDataById API called successfully.", "IN:getQRDataById",getQrDataByIdResponse);
            loggerImpl.startProfiler("preparing getQRDataById API response data.");
        
        
            //created QrCodeParseResponse object.,
            QrCodeParseResponse xsd_QrDataResponse = getQrDataByIdResponse.getReturn().getValue();
	     ...
	     * If response is 101 then no records found.
            ...
            if (ls_responseCode != null && "100".equals(ls_responseCode)) {
				...
				ls_langResCodeMsg=propConfig.getResponseCode("getQRDataById."+ls_responseCode);			
				ls_responseData = common.ofGetErrDataJsonArray(ls_langResCodeMsg,
						...
						propConfig.getMessageOfResCode("getQRDataById."+ls_responseCode,"","(ENP216)"),
						ls_responseMessage, "Currently Service under maintenance so please try later (ENP216).", ls_responseCode, "R");            		            		            	
			}
        }catch (Exception exception) {
			ls_actualErrMsg = common.ofGetTotalErrString(exception, "");            
			loggerImpl.error(logger,"Exception : " + ls_actualErrMsg,"IN:getQRDataById");

			ls_responseData = common.ofGetErrDataJsonArray("999",
					propConfig.getMessageOfResCode("commen.title.999", "Alert."),
					propConfig.getMessageOfResCode("commen.exception.999","","(ENP217)"),
					exception.getMessage(),"Currently Service under maintenance so please try later (ENP217)","0", "R");			

		}finally {
			loggerImpl.stopAndPrintOptLogs(logger,"All API called successfully.","IN:getQRDataById");
			loggerImpl.info(logger,"Response generated and send to client.", "IN:getQRDataById");        	
		}
        return ls_responseData;
    }
}
